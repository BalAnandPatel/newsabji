...
// initialize object
$read_readrefund = new Refunddetails($db);
  
$data = json_decode(file_get_contents("php://input"));
$read_readrefund->sellarId = $data->sellarId;
...
$stmt = $read_readrefund->readRefund();
$num = $stmt->rowCount();
...
if($num>0){
    ...
    // products array
    $read_readrefund_arr=array();
    $read_readrefund_arr["records"]=array();
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        array_push($read_readrefund_arr["records"], $read_readrefund_item);
    }

    // show products data in json format
    echo json_encode($read_readrefund_arr);
